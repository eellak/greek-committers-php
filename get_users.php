...
$time = $time[1] + $time[0]; $start = $time;  $aaz = 0;
$poshoraa = "" ;
...
$user_list = array() ;
 ...
 $fpu = fopen("users_extended.txt", 'w+');
  ...
  $perioxArray = array("Athens+-location:GA+-location:Georgia+-location:OH+-location:ohio", "Thessaloniki", "Patra", "Irakleio", "Larissa", "Patras", "Volos", "Heraklion", "Rhodes", "Rodos", "Ioannina", "Chania", "Chalkis", "Chalkida", "Agrinio", "Katerini", "Trikala", "Serres", "Lamia", "Alexandroupoli", "Kozani", "Kavala", "Veria", "Athina", "Hellas", "Ellada", "Athens,&nbsp;Greece") ; 
 ...
 // DEBUG - DELETE_ME  $zze = 0; // reset location user for extended log
   ...
   // DEBUG - DELETE_ME            $gjt++ ; 
          $djson = json_dec($thejson) ;
            ...
            if($p == 10) $total_count = 0; // unfortunately github's api can't return more than the first 1000 results :(, so we will exit the loop and continue
	   ...
	   $poshoraa .= $aaz++.": ".poshora($start)." - Location: {$perioxh} - Page: {$p} - Total_left: {$total_count} - ResourcesLimit: $rwr \n"  ;	
	   
	   
	   echo $aaz++.": ".poshora($start)." - Location: {$perioxh} - Page: {$p} - Total_left: {$total_count} - ResourcesLimit: $rwr \n"  ;
